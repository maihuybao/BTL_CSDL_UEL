...
from Dashboard import Ui_MainWindow as RealDashboardEx
...
def resource_path(relative_path):
    try:
        ...
        base_path = os.path.abspath("Bang.sql")
...
class LoginEx(QtWidgets.QDialog):
    ...
    def process_login(self):
        ...
        # ---------------------------------------------------------
        # Định vị thư mục gốc "Database" của dự án (thư mục cha của Login_Ex.py)
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

        # Trỏ chính xác đến file database nằm ở thư mục gốc Database
        db_path = os.path.join(base_dir, "Bang.sql")
        if not os.path.exists(db_path):
            self.ui.lblStatus.setText("❌ Không tìm thấy file dữ liệu data_db.sqlite!")
            return
